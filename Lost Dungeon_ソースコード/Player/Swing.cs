...
public class Swing : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject .tag == "Enemy")
        {
            ...
            AttackDamage = (int)Random.Range(GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerStatus>().Attack - MinDamage , GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerStatus>().Attack+MaxDamage);
